...
class CrosswalkStatus:
    ...
    # the timestamp when start stopping for the crosswalk
    stop_time: List[StopTime] = field(default_factory=list)
    # Crosswalks already passed or finished
    finished_crosswalk: List[str] = field(default_factory=list)
...
class OpenSpaceStatus:
    ...
    partitioned_trajectories_index_history: List[str] = field(default_factory=list)
...
class PathDeciderStatus:
    ...
    # Cycle counter for staying in self-lane path
    able_to_use_self_lane_counter: Optional[int] = 0
    # Side-pass directions decided in lane-borrow scenario
    decided_side_pass_direction: List[int] = field(default_factory=list)
...
class LaneFollowCommand:
    ...
    way_point: List[Pose] = field(default_factory=list)
...
class SpeedDeciderStatus:
    ...
    pedestrian_stop_time: List[StopTime] = field(default_factory=list)
...
class StopSignStatus:
    ...
    wait_for_obstacle_id: List[str] = field(default_factory=list)
...
class TrafficLightStatus:
    ...
    current_traffic_light_overlap_id: List[str] = field(default_factory=list)
    # Overlap id of traffic light which is already done
    done_traffic_light_overlap_id: List[str] = field(default_factory=list)
...
class YieldSignStatus:
    ...
    current_yield_sign_overlap_id: List[str] = field(default_factory=list)
    # Overlap id of current yield sign which is already passed
    done_yield_sign_overlap_id: List[str] = field(default_factory=list)
    # Obstacles which the vehicle should stop for
    wait_for_obstacle_id: List[str] = field(default_factory=list)
