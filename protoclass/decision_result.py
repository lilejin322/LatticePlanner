...
class ObjectMotionType:
    ...
    @property
    def static(self) -> Optional[ObjectStatic]:
        return self.motion_tag if isinstance(self.motion_tag, ObjectStatic) else None

    @property
    def dynamic(self) -> Optional[ObjectDynamic]:
        return self.motion_tag if isinstance(self.motion_tag, ObjectDynamic) else None
...
class ObjectDecisionType:
    ...
    @property
    def ignore(self) -> Optional[ObjectIgnore]:
        return self.object_tag if isinstance(self.object_tag, ObjectIgnore) else None

    @property
    def stop(self) -> Optional[ObjectStop]:
        return self.object_tag if isinstance(self.object_tag, ObjectStop) else None

    @property
    def follow(self) -> Optional[ObjectFollow]:
        return self.object_tag if isinstance(self.object_tag, ObjectFollow) else None

    @property
    def yield_(self) -> Optional[ObjectYield]:
        return self.object_tag if isinstance(self.object_tag, ObjectYield) else None

    @property
    def overtake(self) -> Optional[ObjectOvertake]:
        return self.object_tag if isinstance(self.object_tag, ObjectOvertake) else None

    @property
    def nudge(self) -> Optional[ObjectNudge]:
        return self.object_tag if isinstance(self.object_tag, ObjectNudge) else None

    @property
    def avoid(self) -> Optional[ObjectAvoid]:
        return self.object_tag if isinstance(self.object_tag, ObjectAvoid) else None

    @property
    def side_pass(self) -> Optional[ObjectSidePass]:
        return self.object_tag if isinstance(self.object_tag, ObjectSidePass) else None
...
class MainDecision:
    ...
    task: Optional[Union[
        MainCruise, MainStop, MainEmergencyStop, MainChangeLane, MainMissionComplete, MainNotReady, MainParking]] = None
