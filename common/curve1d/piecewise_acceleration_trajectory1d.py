...
class PiecewiseAccelerationTrajectory1d(Curve1d):
    ...
    def AppendSegment(self, a: float, t_duration: float) -> None:
        ...
        assert v1 >= -config_module.FLAGS_numerical_epsilon, "Final velocity must be greater than or equal to -numerical_epsilon"

        delta_s: float = (v0 + v1) * t_duration * 0.5
        s1: float = s0 + delta_s
        t1: float = t0 + t_duration

        assert s1 >= s0 - config_module.FLAGS_numerical_epsilon, "Final position must be greater than or equal to initial position minus numerical_epsilon"
        s1: float = max(s1, s0)
        self._s.append(s1)
        self._v.append(v1)
        self._a.append(a)
