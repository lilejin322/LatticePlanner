...
from protoclass.decision_result import ObjectDecisionType, ObjectIgnore, ObjectStop, ObjectFollow, \
                                      ObjectYield, ObjectOvertake, ObjectNudge, ObjectAvoid, ObjectSidePass
from protoclass.prediction_obstacles import PredictionObstacles, ObstaclePriority
...
class Obstacle:
    ...
    def __init__(self, id: str = "", 
        ...
        self._id: str = id
        if perception_obstacle is None:
        ...
        self._perception_obstacle: PerceptionObstacle = deepcopy(perception_obstacle)
        if isinstance(obstacle_priority, ObstaclePriority.Priority):
            obstacle_priority = Priority[obstacle_priority.name]
        self._obstacle_priority: Priority = obstacle_priority or Priority.NORMAL
        self._perception_id: int = perception_obstacle.id
        self._perception_bounding_box: Box2d = Box2d(Vec2d(perception_obstacle.position.x,
        ...
        self._reference_line_st_boundary: STBoundary = STBoundary()
        self._path_st_boundary: STBoundary = STBoundary()
        self._lateral_decision: ObjectDecisionType = ObjectDecisionType()
        self._longitudinal_decision: ObjectDecisionType = ObjectDecisionType()
        ...
        polygon_point_size = len(perception_obstacle.polygon_point)
        if config_module.FLAGS_use_navigation_mode or polygon_point_size <= 2:
            self.polygon_points.extend(self._perception_bounding_box.GetAllCorners())
        else:
            assert polygon_point_size > 2, f"object {self._id} has less than 3 polygon points"
            for point in perception_obstacle.polygon_point:
                self.polygon_points.append(Vec2d(point.x, point.y))
        tag, polygon = Polygon2d.ComputeConvexHull(self.polygon_points)
        assert tag, f"object {self._id} polygon is not a valid convex hull.\n {perception_obstacle}"
        self._perception_polygon = polygon
        self._is_static: bool = is_static or (self._obstacle_priority == Priority.IGNORE)
        ...
        self._trajectory: Trajectory = deepcopy(trajectory)
        if len(self._trajectory.trajectory_point) > 0:
    ...
    def GetBoundingBox(self, point: TrajectoryPoint) -> Box2d:
        ...
        return Box2d(Vec2d(point.path_point.x, point.path_point.y), point.path_point.theta,
    ...
    def CreateObstacles(predictions: PredictionObstacles) -> List['Obstacle']:
        ...
        for prediction_obstacle in predictions.prediction_obstacle:
            ...
            if not prediction_obstacle.trajectory:
                priority = prediction_obstacle.priority.priority if prediction_obstacle.priority else Priority.NORMAL
                obstacles.append(Obstacle(perception_id, prediction_obstacle.perception_obstacle,
                                          priority, prediction_obstacle.is_static))
                continue
            trajectory_index = 0
            for trajectory in prediction_obstacle.trajectory:
                ...
                obstacles.append(Obstacle(obstacle_id, prediction_obstacle.perception_obstacle,
                                          priority, prediction_obstacle.is_static, trajectory))
                trajectory_index += 1
        return obstacles
    ...
    def CreateStaticVirtualObstacles(id: str, obstacle_box: Box2d) -> 'Obstacle':
        ...
        perception_obstacle = PerceptionObstacle(id=negative_id_signed,
                                                 ...
                                                 height=config_module.FLAGS_virtual_stop_wall_height,
                                                 type=PerceptionObstacle.Type.UNKNOWN_UNMOVABLE,
                                                 tracking_time=1.0)

        corner_points: List[Vec2d] = obstacle_box.GetAllCorners()
    ...
    def PrintPolygonCurve(self) -> None:
        ...
        logger.debug(
            "Obstacle %s polygon: %s",
            self._id,
            [(point.x, point.y) for point in self._perception_polygon.points],
        )
    ...
    def AddLongitudinalDecision(self, decider_tag: str, decision: ObjectDecisionType) -> None:
        ...
        self._longitudinal_decision = self.MergeLongitudinalDecision(self._longitudinal_decision, decision)
        logger.debug(f"{decider_tag} added obstacle {self._id} longitudinal decision: {decision}. The merged decision is {self._longitudinal_decision}")
    ...
    def AddLateralDecision(self, decider_tag: str, decision: ObjectDecisionType) -> None:
        ...
        self._lateral_decision = self.MergeLateralDecision(self._lateral_decision, decision)
        logger.debug(f"{decider_tag} added obstacle {self._id} lateral decision: {decision}. The merged decision is {self._lateral_decision}")
        self._decisions.append(decision)
        self._decider_tags.append(decider_tag)
    ...
    def MinRadiusStopDistance(self) -> float:
        ...
        min_turn_radius: float = config_module.MinSafeTurnRadius
        latteral_diff: float = config_module.EGO_VEHICLE_WIDTH / 2.0 + max(abs(self._sl_boundary.start_l), abs(self._sl_boundary.end_l))
        kEpison: float = 1e-5
        ...
        stop_distance -= config_module.FRONT_EDGE_TO_CENTER
        stop_distance = min(stop_distance, config_module.FLAGS_max_stop_distance_obstacle)
        stop_distance = max(stop_distance, config_module.FLAGS_min_stop_distance_obstacle)
    ...
    def BuildReferenceLineStBoundary(self, reference_line: ReferenceLine, adc_start_s: float) -> None:
        ...
        if self._is_static or not self._trajectory.trajectory_point:
            ...
            success, st_boundary = self.BuildTrajectoryStBoundary(reference_line, adc_start_s)
            if success:
                self._reference_line_st_boundary = st_boundary
    ...
    def IsLongitudinalDecision(decision: ObjectDecisionType) -> bool:
        ...
        return isinstance(decision.object_tag, ObjectIgnore) or isinstance(decision.object_tag, ObjectStop) or isinstance(decision.object_tag, ObjectYield) or \
    ...
    def CheckLaneBlocking(self, reference_line: ReferenceLine) -> None:
        ...
        if reference_line.IsOnLane(self._sl_boundary) and driving_width < config_module.EGO_VEHICLE_WIDTH + config_module.FLAGS_static_obstacle_nudge_l_buffer:
            self._is_lane_blocking = True
            return
        self._is_lane_blocking = False
    ...
    def BuildTrajectoryStBoundary(self, reference_line: ReferenceLine, adc_start_s: float) -> Tuple[bool, STBoundary]:
        ...
        for i in range(1, len(trajectory_points)):
            ...
            has_low: bool = False
            high_s: float = min(object_boundary.end_s + adc_half_length, config_module.FLAGS_st_max_s)
            has_high: bool = False
            while (low_s + st_boundary_delta_s < high_s) and (not has_low or not has_high):
                if not has_low:
                    low_ref = reference_line.GetReferencePoint(low_s)
                    low_box = Box2d(Vec2d(low_ref.x, low_ref.y), low_ref.heading, adc_length, adc_width + config_module.FLAGS_nonstatic_obstacle_nudge_l_buffer)
                    has_low = object_moving_box.HasOverlap(low_box)
                    low_s += st_boundary_delta_s
                if not has_high:
                    high_ref = reference_line.GetReferencePoint(high_s)
                    high_box = Box2d(Vec2d(high_ref.x, high_ref.y), high_ref.heading, adc_length, adc_width + config_module.FLAGS_nonstatic_obstacle_nudge_l_buffer)
                    has_high = object_moving_box.HasOverlap(high_box)
    ...
    @staticmethod
    def IsValidObstacle(perception_obstacle: PerceptionObstacle) -> bool:
        """
        Check if perception obstacle is valid
    ...
    @staticmethod
    def MergeLateralDecision(lhs: ObjectDecisionType, rhs: ObjectDecisionType) -> ObjectDecisionType:
        """
