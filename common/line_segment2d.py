...
class LineSegment2d:
    ...
    def DistanceSquareTo(self, point: Vec2d) -> float:
        ...
        :returns: the square of the shortest distance from points on the line
                  segment to the input point.
        :rtype: float
        """

        if self._length <= kMathEpsilon:
            return point.DistanceSquareTo(self._start)
        x0: float = point.x - self._start.x
        y0: float = point.y - self._start.y
        proj: float = x0 * self._unit_direction.x + y0 * self._unit_direction.y
        if proj <= 0:
            return x0 ** 2 + y0 ** 2
        if proj >= self._length:
            return point.DistanceSquareTo(self._end)
        return (x0 * self._unit_direction.y - y0 * self._unit_direction.x) ** 2
    ...
    def IsPointIn(self, point: Vec2d) -> bool:
        ...
        prod: float = CrossProd(self._start, point, self._end)
