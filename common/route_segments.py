...
class RouteSegments(UserList):
    ...
    def __init__(self, segments: Optional[Iterable[LaneSegment]] = None):
        """
        Constructor
        """
    ...
    def GetProjection(self, *args) -> Tuple[bool, SLPoint, LaneWaypoint]:
        ...
        elif len(args) == 2 and isinstance(args[0], Vec2d) and (
            args[1] is None or isinstance(args[1], (float, int))
        ):
            """
            :param Vec2d point: a Vec2d point
    ...
    def CanDriveFrom(self, waypoint: LaneWaypoint) -> bool:
        ...
        waypoint_left_width, waypoint_right_width, _ = waypoint.lane.GetWidth(waypoint.s)
        segment_left_width, segment_right_width, _ = segment_waypoint.lane.GetWidth(segment_waypoint.s)
    ...
    def SetProperties(self, other: 'RouteSegments') -> None:
        ...
        self._is_neighbor = deepcopy(other.IsNeighborSegment())

    @staticmethod
    def WithinRoutingLaneSegment(lane_segment: LaneSegment, waypoint) -> bool:
        """Check if a routing LaneWaypoint lies within a lane segment."""
        lane_id = getattr(waypoint, "id", None)
        waypoint_s = getattr(waypoint, "s", None)
        if lane_id is None or waypoint_s is None or lane_segment.lane is None:
            return False
        seg_lane_id = getattr(lane_segment.lane.id, "id", lane_segment.lane.id)
        return (
            str(seg_lane_id) == str(lane_id)
            and lane_segment.start_s - kSegmentationEpsilon <= waypoint_s
            and lane_segment.end_s + kSegmentationEpsilon >= waypoint_s
        )

    @staticmethod
    def WithinRoutingLaneSegmentPair(routing_lane_segment, waypoint: LaneWaypoint) -> bool:
        """Check if a map LaneWaypoint lies within a routing LaneSegment."""
        if waypoint.lane is None:
            return False
        lane_id = getattr(routing_lane_segment, "id", None)
        start_s = getattr(routing_lane_segment, "start_s", None)
        end_s = getattr(routing_lane_segment, "end_s", None)
        seg_lane_id = getattr(waypoint.lane.id, "id", waypoint.lane.id)
        return (
            str(seg_lane_id) == str(lane_id)
            and start_s - kSegmentationEpsilon <= waypoint.s
            and end_s + kSegmentationEpsilon >= waypoint.s
        )

    @staticmethod
    def WithinLaneSegment(lane_segment: LaneSegment, waypoint: LaneWaypoint) -> bool:
        """
