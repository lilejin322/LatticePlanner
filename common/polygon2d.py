...
from common.geometry_utils import CrossProd, WrapAngle

if TYPE_CHECKING:
    from common.box2d import Box2d

# Backward-compatible re-exports.
__all__ = ["Polygon2d", "kMathEpsilon", "CrossProd", "WrapAngle"]


def _is_box2d(obj: object) -> bool:
    from common.box2d import Box2d

    return isinstance(obj, Box2d)


def _segment_point_distance(segment: LineSegment2d, point: Vec2d) -> float:
    return segment.DistanceTo(point)[0]
...
class Polygon2d:
    ...
    def __init__(self, *args):
        ...
        if len(args) == 0:
            ...
            box: Box2d = args[0]
            self._points = list(box.GetAllCorners())
    ...
    def DistanceToBoundary(self, point: Vec2d) -> float:
        ...
        for i in range(self._num_points):
            distance = min(distance, _segment_point_distance(self._line_segments[i], point))
        return distance
    ...
    def DistanceTo(self, *args) -> float:
        ...
        if isinstance(args[0], Vec2d):
            ...
            for i in range(self._num_points):
                distance = min(distance, _segment_point_distance(line_segment, self._points[i]))
            return distance
    ...
    def ComputeConvexHull(points: List[Vec2d]) -> Tuple[bool, 'Polygon2d']:
        ...
        if n < 3:
            return False, None
        sorted_indices: List[int] = list(range(n))
        sorted_indices.sort(key=lambda idx: (points[idx].x, points[idx].y))

        lower: List[int] = []
        for idx in sorted_indices:
            while len(lower) >= 2 and CrossProd(points[lower[-2]], points[lower[-1]], points[idx]) <= kMathEpsilon:
                lower.pop()
            lower.append(idx)

        upper: List[int] = []
        for idx in reversed(sorted_indices):
            while len(upper) >= 2 and CrossProd(points[upper[-2]], points[upper[-1]], points[idx]) <= kMathEpsilon:
                upper.pop()
            upper.append(idx)
        ...
        if len(hull_indices) < 3:
            return False, None
        result_points: List[Vec2d] = [points[i] for i in hull_indices]
    ...
    def HasOverlap(self, *args) -> bool:
        ...
        if isinstance(args[0], Polygon2d):
            ...
            polygon: Polygon2d = args[0]
            assert len(self._points) >= 3, f"len(self._points) should be greater than 3, but got {len(self._points)}"
            assert polygon.num_points >= 3, f"polygon.num_points should be greater than 3, but got {polygon.num_points}"
