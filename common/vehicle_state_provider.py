...
class VehicleStateProvider:
    ...
    def Update(self, *args):
        ...
        if isinstance(args[0], LocalizationEstimate) and isinstance(args[1], Chassis):
            ...
            if chassis.speed_mps is not None:
                ...
                if not config_module.FLAGS_reverse_heading_vehicle_state and self._vehicle_state.gear == Chassis.GearPosition.GEAR_REVERSE:
        ...
        if isinstance(args[0], str) and isinstance(args[1], str):
            ...
            return Status(
                ...
                f"Loading vehicle state from files is not supported: {localization_file}, {chassis_file}",
            )
    ...
    def ComputeCOMPosition(self, rear_to_com_distance: float) -> Vec2d:
        ...
        if (config_module.FLAGS_state_transform_to_com_reverse and self._vehicle_state.gear == Chassis.GearPosition.GEAR_REVERSE) or \
           (config_module.FLAGS_state_transform_to_com_drive and self._vehicle_state.gear == Chassis.GearPosition.GEAR_DRIVE):
    ...
    def ConstructExceptLinearVelocity(self, localization: LocalizationEstimate) -> bool:
        ...
        if config_module.FLAGS_enable_map_reference_unify:
            if localization.pose.angular_velocity_vrf is None:
                logger.error("localization.pose().angular_velocity_vrf must not be None when config_module.FLAGS_enable_map_reference_unify is true.")
                return False
            ...
            if localization.pose.linear_acceleration_vrf is None:
                logger.error("localization.pose().linear_acceleration_vrf must not be None when config_module.FLAGS_enable_map_reference_unify is true.")
                return False
