...
from common.geometry_utils import CrossProd
...
class STBoundary(Polygon2d):
    ...
    def __init__(self, point_pairs: List[Tuple[STPoint, STPoint]] = None, is_accurate_boundary: bool = False):
        """
        Constructors:
        ...
        if isinstance(point_pairs, str):
            self._id = point_pairs
            point_pairs = []
    ...
    def GetUnblockSRange(self, curr_time: float) -> Tuple[bool, float, float]:
        ...
        s_upper = config_module.FLAGS_speed_lon_decision_horizon
        s_lower = 0.0
    ...
    def GetBoundarySRange(self, curr_time: float) -> Tuple[bool, float, float]:
        ...
        s_upper = min(s_upper, config_module.FLAGS_speed_lon_decision_horizon)
    ...
    def PrintDebug(self, suffix: str = ""):
        ...
        logger.debug(
            "STBoundary%s type=%s id=%s s=[%.3f, %.3f] t=[%.3f, %.3f] lower=%s upper=%s",
            suffix,
            self._boundary_type.name,
            self._id,
            self._min_s,
            self._max_s,
            self._min_t,
            self._max_t,
            [(point.s, point.t) for point in self._lower_points],
            [(point.s, point.t) for point in self._upper_points],
        )
    ...
    def upper_left_point(self) -> STPoint:
        ...
        if self._upper_left_point is not None:
    ...
    def upper_right_point(self) -> STPoint:
        ...
        if self._upper_right_point is not None:
    ...
    def bottom_left_point(self) -> STPoint:
        ...
        if self._bottom_left_point is not None:
    ...
    def bottom_right_point(self) -> STPoint:
        ...
        if self._bottom_right_point is not None:
    ...
    def set_upper_left_point(self, st_point: STPoint) -> None:
        ...
        self._min_s = min(self._min_s, st_point.s)
        self._max_s = max(self._max_s, st_point.s)
        self._min_t = min(self._min_t, st_point.t)
        self._max_t = max(self._max_t, st_point.t)
