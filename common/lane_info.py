...
def MakeMapId(id_str) -> Id:
    return Id(getattr(id_str, "id", id_str))

@dataclass
class LaneSegmentBox:
    _aabox: AABox2d
    _lane: "LaneInfo"
    _segment: LineSegment2d
    _id: int
    ...
    def DistanceTo(self, point: Vec2d) -> float:
        distance, _ = self._segment.DistanceTo(point)
        return distance
    ...
    def object(self) -> "LaneInfo":
        return self._lane

    def geo_object(self) -> LineSegment2d:
...
class LaneInfo:
        ...
        for i in range(seg_num):
            if heading is not None:
                if abs(AngleDiff(self._segments[i].heading(), heading)) >= math.pi / 2.0:
                    continue
            distance = self._segments[i].DistanceSquareTo(point)
        ...
        if self._lane.type is not None:
            if self._lane.type == Lane.LaneType.CITY_DRIVING:
                for p in self._sampled_left_width:
                    if p[1] < config_module.FLAGS_half_vehicle_width:
                        logger.error(f"_lane[id = {self._lane.id.id}]. _sampled_left_width[{p[1]}] is too small. It should be larger than half vehicle width[{config_module.FLAGS_half_vehicle_width}].")
                for p in self._sampled_right_width:
                    if p[1] < config_module.FLAGS_half_vehicle_width:
                        logger.error(f"_lane[id = {self._lane.id.id}]. _sampled_right_width[{p[1]}] is too small. It should be larger than half vehicle width[{config_module.FLAGS_half_vehicle_width}].")
    ...
    def PostProcess(self, map_instance) -> None:
        """
        Post process method
        """
    ...
    def UpdateOverlaps(self, map_instance) -> None:
        """
        ...
        for overlap_id in self._overlap_ids:
            ...
            for obj in getattr(overlap.overlap, "objects", []):
                object_id = obj.id.id
    ...
    def CreateKDTree(self) -> None:
        ...
        for id in range(len(self._segments)):
            ...
            self._segment_box_list.append(LaneSegmentBox(AABox2d(segment.start, segment.end), self, segment, id))
