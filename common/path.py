...
from protoclass.overlap import LaneOverlapInfo
...
class Path:
    ...
    def InitLaneSegments(self) -> None:
        ...
        if not self._lane_segments:
            return
        self._lane_accumulated_s = [0.0] * len(self._lane_segments)
    ...
    def InitWidth(self) -> None:
        ...
        sample_s: float = 0.0
        for _ in range(self._num_sample_points):
            point = self.GetSmoothPoint(sample_s)
            if not point.lane_waypoints:
                half_width = config_module.FLAGS_default_lane_width / 2.0
                self._lane_left_width.append(half_width)
                self._lane_right_width.append(half_width)
                self._road_left_width.append(half_width)
                self._road_right_width.append(half_width)
                logger.warning(f"path point: {point} has invalid width.")
            else:
                waypoint = point.lane_waypoints[0]
                assert waypoint.lane, f"waypoint.lane is None at s={sample_s}"
                left_width, right_width, _ = waypoint.lane.GetWidth(waypoint.s)
                self._lane_left_width.append(left_width - waypoint.l)
                self._lane_right_width.append(right_width + waypoint.l)
                left_width, right_width, _ = waypoint.lane.GetRoadWidth(waypoint.s)
                self._road_left_width.append(left_width - waypoint.l)
                self._road_right_width.append(right_width + waypoint.l)
    ...
    def InitOverlaps(self) -> None:
        ...
        self._lane_overlaps = self.GetAllOverlaps(lambda lane: lane.cross_lanes)
        self._signal_overlaps = self.GetAllOverlaps(lambda lane: lane.signals)
        self._yield_sign_overlaps = self.GetAllOverlaps(lambda lane: lane.yield_signs)
        self._stop_sign_overlaps = self.GetAllOverlaps(lambda lane: lane.stop_signs)
        self._crosswalk_overlaps = self.GetAllOverlaps(lambda lane: lane.crosswalks)
        self._junction_overlaps = self.GetAllOverlaps(lambda lane: lane.junctions)
        self._pnc_junction_overlaps = self.GetAllOverlaps(lambda lane: lane.pnc_junctions)
        self._clear_area_overlaps = self.GetAllOverlaps(lambda lane: lane.clear_areas)
        self._speed_bump_overlaps = self.GetAllOverlaps(lambda lane: lane.speed_bumps)
        self._parking_space_overlaps = self.GetAllOverlaps(lambda lane: lane.parking_spaces)
    ...
    def GetSmoothPoint(self, param: Any) -> MapPathPoint:
        ...
        if isinstance(param, InterpolatedIndex):
            ...
            if abs(index.offset) > kMathEpsilon:
                ...
                if (
                    index.id < self._num_segments
                    and index.id < len(self._lane_segments_to_next_point)
                    and ref_point.lane_waypoints
                ):
    ...
    def GetProjection(self, point: Vec2d, heading: float = None) -> Tuple[bool, float, float, float]:
        ...
        if self._use_path_approximation:
            return self._approximation.GetProjection(self, point)
        assert self._num_points >= 2, "num_points is less than 2"
        min_distance = float("inf")
        min_index: int = 0
    ...
    def GetAllOverlaps(self, get_overlaps_from_lane) -> List[PathOverlap]:
        """
        Get all overlaps using a lane overlap accessor, matching hdmap::Path::GetAllOverlaps.

        :param get_overlaps_from_lane: callable returning overlap list for a LaneInfo
        ...
        for lane_segment in self._lane_segments:
            if not lane_segment.lane:
                continue
            for overlap in get_overlaps_from_lane(lane_segment.lane):
                overlap_info = overlap.GetObjectOverlapInfo(lane_segment.lane.id)
                if overlap_info is None or not isinstance(overlap_info.overlap_info, LaneOverlapInfo):
                    continue
                lane_overlap_info = overlap_info.overlap_info
                if lane_overlap_info.start_s <= lane_segment.end_s and lane_overlap_info.end_s >= lane_segment.start_s:
                    ref_s = s - lane_segment.start_s
                    adjusted_start_s = max(lane_overlap_info.start_s, lane_segment.start_s) + ref_s
                    adjusted_end_s = min(lane_overlap_info.end_s, lane_segment.end_s) + ref_s
                    for obj in overlap.overlap.objects:
                        if obj.id.id != lane_segment.lane.id.id:
                            overlaps_by_id[obj.id.id].append((adjusted_start_s, adjusted_end_s))
