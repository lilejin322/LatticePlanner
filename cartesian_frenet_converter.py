...
class CartesianFrenetConverter:
    ...
    def CalculateLateralDerivative(
        rtheta: float, theta: float, l: float, rkappa: float
    ) -> float:
        return (1.0 - rkappa * l) * math.tan(theta - rtheta)

    @staticmethod
    def CalculateSecondOrderLateralDerivative(
        rtheta: float,
        theta: float,
        rkappa: float,
        kappa: float,
        ...
        if abs(cos_theta_diff) < 1e-8:
            return 0.0
        return (
            -(rdkappa * l + rkappa * dl) * math.tan(theta_diff)
            + (1.0 - rkappa * l)
            / (cos_theta_diff * cos_theta_diff)
