...
class ReferenceLineInfo:
    ...
    def __init__(self, vehicle_state: VehicleState=None, adc_planning_point: TrajectoryPoint=None, reference_line: ReferenceLine=None, segments: RouteSegments=None):
        ...
        self._vehicle_state = deepcopy(vehicle_state)
        self._adc_planning_point = deepcopy(adc_planning_point)
        self._reference_line = deepcopy(reference_line)
        self._lanes: RouteSegments = deepcopy(segments)
        # this is the number that measures the goodness of this reference line.
        # The lower the better.
        self._cost: float = 0.0
        self._is_drivable: bool = True
        self._path_decision: PathDecision = PathDecision()
        ...
        self._planning_target: PlanningTarget = PlanningTarget(cruise_speed=config_module.FLAGS_default_cruise_speed)
        ...
        self._index: int = 0
        self._debug = Debug(planning_data=PlanningData(path=[]))

    def Init(self, obstacles: List[Obstacle], target_speed: float = None) -> bool:
        """
        ...
        if target_speed is None:
            target_speed = config_module.FLAGS_default_cruise_speed

        # stiching point
        path_point = self._adc_planning_point.path_point
        position: Vec2d = Vec2d(path_point.x, path_point.y)
        vec_to_center: Vec2d = Vec2d((config_module.FRONT_EDGE_TO_CENTER - config_module.BACK_EDGE_TO_CENTER) / 2.0, (config_module.LEFT_EDGE_TO_CENTER - config_module.RIGHT_EDGE_TO_CENTER) / 2.0)
        center: Vec2d = position + vec_to_center.rotate(path_point.theta)
        box: Box2d = Box2d(center, path_point.theta, config_module.EGO_VEHICLE_LENGTH, config_module.EGO_VEHICLE_WIDTH)
        # realtime vehicle position
        vehicle_position: Vec2d = Vec2d(self._vehicle_state.x, self._vehicle_state.y)
        vehicle_center: Vec2d = vehicle_position + vec_to_center.rotate(self._vehicle_state.heading)
        vehicle_box: Box2d = Box2d(vehicle_center, self._vehicle_state.heading, config_module.EGO_VEHICLE_LENGTH, config_module.EGO_VEHICLE_WIDTH)
        self._adc_sl_boundary = SLBoundary()
        ...
        for speed_bump in getattr(map_path, "speed_bump_overlaps", []):
            # -1 and + 1.0 are added to make sure it can be sampled.
            self._reference_line.AddSpeedLimit(speed_bump.start_s - 1.0, speed_bump.end_s + 1.0, config_module.FLAGS_speed_bump_speed_limit)
    ...
    def AddObstacles(self, obstacles: List[Obstacle]) -> bool:
        ...
        if isinstance(obstacles, dict):
            obstacles = list(obstacles.values())

        if config_module.FLAGS_use_multi_thread_to_add_obstacles:
    ...
    def AddObstacle(self, obstacle: Obstacle) -> Obstacle:
        ...
        if self.IsIrrelevantObstacle(mutable_obstacle):
            ...
            st_boundary = mutable_obstacle.reference_line_st_boundary()
            if st_boundary and not st_boundary.IsEmpty():
                logger.debug(f"Reference line st boundary: t[{st_boundary.min_t}, {st_boundary.max_t}] s[{st_boundary.min_s}, {st_boundary.max_s}]")
    ...
    def GetBaseCruiseSpeed(self) -> float:
        ...
        return self._base_cruise_speed if self._base_cruise_speed > 0.0 else config_module.FLAGS_default_cruise_speed
    ...
    def GetNeighborLaneInfo(self, lane_type: LaneType, s: float) -> Tuple[bool, str, float]:
        ...
        if hdmap_util is None:
            return False, "", float("inf")
        neighbor_lane = hdmap_util.BaseMapPtr().GetLaneById(lane_id)
    ...
    def SetPathData(self, path_data: PathData) -> None:
        self._path_data = path_data

    def SetSpeedData(self, speed_data: SpeedData) -> None:
        self._speed_data = speed_data

    @property
    def debug(self):
        if self._debug.planning_data is None:
            self._debug.planning_data = PlanningData()
        return self._debug
    ...
    def CombinePathAndSpeedProfile(self, relative_time: float, start_s: float, discretized_trajectory: DiscretizedTrajectory) -> bool:
        ...
        kDenseTimeResoltuion = config_module.FLAGS_trajectory_time_min_interval
        kSparseTimeResolution = config_module.FLAGS_trajectory_time_max_interval
        kDenseTimeSec = config_module.FLAGS_trajectory_time_high_density_period
    ...
    def AdjustTrajectoryWhichStartsFromCurrentPos(planning_start_point: TrajectoryPoint,
        ...
        # use varied resolution to reduce data load but also provide enough data
        # point for control module
        kDenseTimeResoltuion: float = config_module.FLAGS_trajectory_time_min_interval
        kSparseTimeResolution: float = config_module.FLAGS_trajectory_time_max_interval
        kDenseTimeSec: float = config_module.FLAGS_trajectory_time_high_density_period
    ...
    def IsChangeLanePath(self) -> bool:
        ...
        return bool(self._lanes is not None and not self._lanes.IsOnSegment())
    ...
    def ExportDecision(self, planning_context: PlanningContext) -> Tuple[DecisionResult, PlanningContext]:
        ...
        decision_result, planning_context = self.MakeDecision(planning_context)
    ...
    def SetTurnSignalBasedOnLaneTurnType(self, vehicle_signal: VehicleSignal) -> None:
        ...
        path_label = self._path_data.path_label if self._path_data is not None else ""
        if "left" in path_label:
            vehicle_signal.turn_signal = VehicleSignal.TurnSignal.TURN_LEFT
            return
        if "right" in path_label:
            vehicle_signal.turn_signal = VehicleSignal.TurnSignal.TURN_RIGHT
            return
        ...
        for seg in self.Lanes():
            if route_s > adc_s + config_module.FLAGS_turn_signal_distance:
                break
            route_s += seg.end_s - seg.start_s
            if route_s < adc_s:
                continue
            turn = getattr(seg.lane.lane, "turn", None)
    ...
    def ExportVehicleSignal(self, decision_result: DecisionResult) -> VehicleSignal:
        ...
        if decision_result.vehicle_signal is None:
            from protoclass.decision_result import VehicleSignal

            decision_result.vehicle_signal = VehicleSignal()
        if self._vehicle_signal is not None:
    ...
    def MakeDecision(self, planning_context: PlanningContext) -> Tuple[DecisionResult, PlanningContext]:
        ...
        decision_result = DecisionResult(
            main_decision=MainDecision(task=MainCruise()),
            object_decision=ObjectDecisions(),
        )
    ...
    def MakeMainStopDecision(self, decision_result: DecisionResult) -> int:
        ...
        if stop_obstacle is not None:
            main_stop = MainStop(
                ...
                reason="stop by " + stop_obstacle.Id(),
                stop_point=deepcopy(stop_decision.stop_point),
                stop_heading=stop_decision.stop_heading,
            )
            decision_result.main_decision.task = main_stop
    ...
    def MakeMainMissionCompleteDecision(self, decision_result: DecisionResult, planning_context: PlanningContext) -> None:
        ...
        if main_stop.reason_code not in (
            StopReasonCode.STOP_REASON_DESTINATION,
            StopReasonCode.STOP_REASON_PULL_OVER,
        ):
            return
        distance_destination: float = self.SDistanceToDestination()
        if distance_destination > config_module.FLAGS_destination_check_distance:
            return
    ...
    def MakeEStopDecision(self) -> DecisionResult:
        ...
        decision_result = DecisionResult(
            main_decision=MainDecision(task=MainEmergencyStop()),
            object_decision=ObjectDecisions(),
        )
        ...
        for obstacle in self._path_decision.obstacles.values():
            ...
            decision_result.object_decision.decision.append(object_decision)
    ...
    def SetObjectDecisions(self, decision_result: DecisionResult) -> None:
        ...
        if decision_result.object_decision is None:
            decision_result.object_decision = ObjectDecisions()
