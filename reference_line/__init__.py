...
def _to_map_path_point(point: MapPathPoint) -> MapPathPoint:
    return MapPathPoint(point, point.heading, list(point.lane_waypoints))
...
class ReferenceLine:
    ...
    def __init__(self, *args):
        ...
        if len(args) == 1 and isinstance(args[0], ReferenceLine):
            ...
            self._map_path = MapPath([_to_map_path_point(p) for p in self._reference_points])
            ...
            assert self._map_path.num_points == len(self._reference_points), \
                   "Number of points in self._map_path does not match the size of self._reference_points"
    ...
    def Stitch(self, other: 'ReferenceLine') -> bool:
        ...
        if not other.reference_points:
            ...
            return True
        first_point = self._reference_points[0]
    ...
    def GetReferencePoint(self, *args) -> ReferencePoint:
        ...
        if len(args) == 1 and isinstance(args[0], float):
            ...
            if s < accumulated_s[0] - 1e-2:
                logger.debug(f"The requested s: {s} < {accumulated_s[0]}.")
                return self._reference_points[0]
            if s > accumulated_s[-1] + 1e-2:
                logger.debug(f"The requested s: {s} > {accumulated_s[-1]}.")
    ...
    def ToFrenetFrame(self, traj_point: TrajectoryPoint) -> Tuple[List[float], List[float]]:
        ...
        ok, sl_point = self.XYToSL(Vec2d(traj_point.path_point.x, traj_point.path_point.y))
    ...
    def GetSLBoundary(self, *args) -> bool:
        ...
        if isinstance(args[0], Box2d):
            ...
            if isinstance(polygon, Polygon2d):
                points = polygon.GetAllVertices()
            elif hasattr(polygon, "GetAllVertices"):
                points = polygon.GetAllVertices()
            elif hasattr(polygon, "point"):
    ...
    def XYToSL(self, *args) -> Tuple[bool, SLPoint]:
        ...
        if isinstance(args[0], Vec2d):
            ...
            l = 0.0
            if warm_start_s < 0.0:
                tag, s, l, _ = self._map_path.GetProjection(xy_point)
                if not tag:
    ...
    def GetRoadType(self, s: float):
        ...
        hdmap_util = globals().get("HDMapUtil")
        if hdmap_util is None:
            return RoadType.UNKNOWN
        hdmap = hdmap_util.BaseMap()
        if hdmap is None:
            return RoadType.UNKNOWN
        ...
        roads = hdmap.GetRoads(point, 4.0) if hasattr(hdmap, "GetRoads") else []
    ...
    def __str__(self) -> str:
        ...
        limit: int = min(len(self._reference_points), config_module.FLAGS_trajectory_point_num_for_debug)
    ...
    def GetSpeedLimitFromS(self, s: float) -> float:
        ...
        speed_limit: float = config_module.FLAGS_planning_upper_speed_limit
        ...
        for lane_waypoint in map_path_point.lane_waypoints:
            ...
            speed_limit = min(lane_waypoint.lane.lane.speed_limit, speed_limit)

        if not speed_limit_found:
            # use default speed limit based on road_type
            speed_limit = config_module.FLAGS_default_city_road_speed_limit
            road_type = self.GetRoadType(s)
            if road_type == RoadType.HIGHWAY:
                speed_limit = config_module.FLAGS_default_highway_speed_limit
