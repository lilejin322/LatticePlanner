...
from protoclass.planning_target import PlanningTarget
...
from common.curve1d.piecewise_acceleration_trajectory1d import PiecewiseAccelerationTrajectory1d
from trajectory_generation.piecewise_braking_trajectory_generator import PiecewiseBrakingTrajectoryGenerator
from protoclass.speed_point import SpeedPoint
...
class TrajectoryEvaluator:
    ...
    def __init__(self, 
        ...
        # Note that cost_queue is List[weight, counter, Tuple[lon_trajectory, lat_trajectory]]
        self.cost_queue: List[Tuple[float, int, Tuple[Curve1d, Curve1d]]] = []
        self._pair_index = 0
        start_time = 0.0
        end_time = config_module.FLAGS_trajectory_time_length
        self.path_time_intervals = self.path_time_graph.GetPathBlockingIntervals(
            start_time, end_time, config_module.FLAGS_trajectory_time_resolution)
        ...
        for lon_trajectory in lon_trajectories:
            ...
            if init_s[0] < stop_point and lon_end_s + config_module.FLAGS_lattice_stop_buffer > stop_point:
                continue
            ...
            for lat_trajectory in lat_trajectories:
                ...
                heapq.heappush(
                    self.cost_queue,
                    (cost, self._pair_index, (lon_trajectory, lat_trajectory)),
                )
                self._pair_index += 1
    ...
    def next_top_trajectory_pair(self) -> Tuple[Curve1d, Curve1d]:
        ...
        top = heapq.heappop(self.cost_queue)[2]
    ...
    def Evaluate(self, planning_target: PlanningTarget, lon_trajectory: Curve1d, lat_trajectory: Curve1d,
        ...
        evaluation_horizon: float = min(config_module.FLAGS_speed_lon_decision_horizon,
                                        lon_trajectory.Evaluate(0, lon_trajectory.ParamLength()))
        s_values: List[float] = [s for s in np.arange(0.0, evaluation_horizon, config_module.FLAGS_trajectory_space_resolution)]
        ...
        total_cost = (lon_objective_cost * config_module.FLAGS_weight_lon_objective +
                      lon_jerk_cost * config_module.FLAGS_weight_lon_jerk +
                      lon_collision_cost * config_module.FLAGS_weight_lon_collision +
                      centripetal_acc_cost * config_module.FLAGS_weight_centripetal_acceleration +
                      lat_offset_cost * config_module.FLAGS_weight_lat_offset +
                      lat_comfort_cost * config_module.FLAGS_weight_lat_comfort)
        return total_cost
    ...
    def LatOffsetCost(lat_trajectory: Curve1d, s_values: List[float]) -> float:
        ...
        for s in s_values:
            ...
            cost: float = lat_offset / config_module.FLAGS_lat_offset_bound
            if lat_offset * lat_offset_start < 0.0:
                cost_sqr_sum += cost * cost * config_module.FLAGS_weight_opposite_side_offset
                cost_abs_sum += abs(cost) * config_module.FLAGS_weight_opposite_side_offset
            else:
                cost_sqr_sum += cost * cost * config_module.FLAGS_weight_same_side_offset
                cost_abs_sum += abs(cost) * config_module.FLAGS_weight_same_side_offset
        return cost_sqr_sum / (cost_abs_sum + config_module.FLAGS_numerical_epsilon)
    ...
    def LatComfortCost(self, lon_trajectory: Curve1d, lat_trajectory: Curve1d) -> float:
        ...
        max_cost: float = 0.0
        t: float = 0.0
        while t < config_module.FLAGS_trajectory_time_length:
    ...
    def LonComfortCost(lon_trajectory: Curve1d) -> float:
        ...
        while t < config_module.FLAGS_trajectory_time_length:
            ...
            cost: float = jerk / config_module.FLAGS_longitudinal_jerk_upper_bound
    ...
    def LonObjectiveCost(lon_trajectory: Curve1d, planning_target: PlanningTarget, ref_s_dots: List[float]) -> float:
        ...
        for i in range(len(ref_s_dots)):
            t: float = float(i) * config_module.FLAGS_trajectory_time_resolution
            cost: float = ref_s_dots[i] - lon_trajectory.Evaluate(1, t)
            speed_cost_sqr_sum += t * t * abs(cost)
            speed_cost_weight_sum += t * t
        
        speed_cost: float = speed_cost_sqr_sum / (speed_cost_weight_sum + config_module.FLAGS_numerical_epsilon)
        dist_travelled_cost: float = 1.0 / (1.0 + dist_s)
        return (speed_cost * config_module.FLAGS_weight_target_speed + dist_travelled_cost * config_module.FLAGS_weight_dist_travelled) / (config_module.FLAGS_weight_target_speed + config_module.FLAGS_weight_dist_travelled)
    ...
    def LonCollisionCost(self, lon_trajectory: Curve1d) -> float:
        ...
        for i in range(len(self.path_time_intervals)):
            ...
            sigma: float = config_module.FLAGS_lon_collision_cost_std
            for m in pt_interval:
                dist: float = 0.0
                if traj_s < m[0] - config_module.FLAGS_lon_collision_yield_buffer:
                    dist = m[0] - config_module.FLAGS_lon_collision_yield_buffer - traj_s
                elif traj_s > m[1] + config_module.FLAGS_lon_collision_overtake_buffer:
                    dist = traj_s - m[1] - config_module.FLAGS_lon_collision_overtake_buffer
    ...
    def CentripetalAccelerationCost(self, lon_trajectory: Curve1d) -> float:
        ...
        while t < config_module.FLAGS_trajectory_time_length:
            ...
            t += config_module.FLAGS_trajectory_time_resolution
        return centripetal_acc_sqr_sum / (centripetal_acc_sum + config_module.FLAGS_numerical_epsilon)
    ...
    def ComputeLongitudinalGuideVelocity(self, planning_target: PlanningTarget) -> List[float]:
        ...
        if planning_target.stop_point is None:
            lon_traj: PiecewiseAccelerationTrajectory1d = PiecewiseAccelerationTrajectory1d(self.init_s[0], cruise_v)
            lon_traj.AppendSegment(0.0, config_module.FLAGS_trajectory_time_length + config_module.FLAGS_numerical_epsilon)
            ...
            if dist_s < config_module.FLAGS_numerical_epsilon:
            ...
            a_comfort: float = config_module.FLAGS_longitudinal_acceleration_upper_bound * config_module.FLAGS_comfort_acceleration_factor
            d_comfort: float = -config_module.FLAGS_longitudinal_acceleration_lower_bound * config_module.FLAGS_comfort_acceleration_factor
            ...
            lon_ref_trajectory: Curve1d = PiecewiseBrakingTrajectoryGenerator.Generate(
                                               ...
                                               config_module.FLAGS_trajectory_time_length + config_module.FLAGS_numerical_epsilon)
