...
class TrajectoryCombiner:
    ...
    def Combine(reference_line: List[PathPoint], lon_trajectory: Curve1d, lat_trajectory: Curve1d, init_relative_time: float) -> DiscretizedTrajectory:
        ...
        last_s: float = -config_module.FLAGS_numerical_epsilon
        t_param: float = 0.0
        while t_param < config_module.FLAGS_trajectory_time_length:
            ...
            s_dot: float = max(config_module.FLAGS_numerical_epsilon, lon_trajectory.Evaluate(1, t_param))
            ...
            if prev_trajectory_point is not None and prev_trajectory_point.x is not None and prev_trajectory_point.y is not None:
            ...
            t_param += config_module.FLAGS_trajectory_time_resolution
