...
from common.curve1d.quintic_polynomial_curve1d import QuinticPolynomialCurve1d
...
from trajectory_generation.lateral_osqp_optimizer import CreateLateralOptimizer
...
class Trajectory1dGenerator:
    ...
    def GenerateTrajectoryBundles(self, planning_target: PlanningTarget) -> Tuple[List[Curve1d], List[Curve1d]]:
        """
        Generate longitudinal and lateral trajectory bundles.

        :param PlanningTarget planning_target: planning target
        :returns: (lon_trajectory_bundle, lat_trajectory_bundle)
        """

        lon_trajectory_bundle: List[Curve1d] = []
        lat_trajectory_bundle: List[Curve1d] = []
        ...
        return lon_trajectory_bundle, lat_trajectory_bundle
    ...
    def GenerateLateralTrajectoryBundle(self, lat_trajectory_bundle: List[Curve1d]) -> None:
        ...
        if not config_module.FLAGS_lateral_optimization:
            ...
            s_max: float = s_min + config_module.FLAGS_max_s_lateral_optimization

            delta_s: float = config_module.FLAGS_default_delta_s_lateral_optimization

            lateral_bounds: List[Tuple[float, float]] = self.path_time_graph.GetLateralBounds(s_min, s_max, delta_s)
    ...
    def GenerateTrajectory1DBundle(init_state: List[float], end_conditions: List[Tuple[List[float], float]], trajectory_bundle: List[Curve1d], order: int) -> None:
        ...
        for end_condition in end_conditions:
            if order == 4:
                trajectory1d: Curve1d = CreateLatticeTrajectory1d(QuarticPolynomialCurve1d(
                    init_state, [end_condition[0][1], end_condition[0][2]], end_condition[1]))
            elif order == 5:
                trajectory1d: Curve1d = CreateLatticeTrajectory1d(QuinticPolynomialCurve1d(
