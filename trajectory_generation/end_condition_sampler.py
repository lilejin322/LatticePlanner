...
from behavior.feasible_region import FeasibleRegion
...
class EndConditionSampler:
    ...
    def SampleLonEndConditionsForCruising(self, ref_cruise_speed: float) -> List[Tuple[List[float], float]]:
        ...
        assert config_module.FLAGS_num_velocity_sample > 1, "Number of velocity samples must be greater than 1"
        ...
        for i in range(1, num_of_time_samples):
            ...
            time_samples[i] = config_module.FLAGS_trajectory_time_length * ratio
        time_samples[0] = config_module.FLAGS_polynomial_minimal_param
    ...
    def SampleLonEndConditionsForPathTimePoints(self) -> List[Tuple[List[float], float]]:
        ...
        for sample_point in sample_points:
            if sample_point.path_time_point.t < config_module.FLAGS_polynomial_minimal_param:
                continue
    ...
    def QueryFollowPathTimePoints(self, obstacle_id: str, sample_points: List[SamplePoint]) -> None:
        ...
        follow_path_time_points: List[STPoint] = self.path_time_graph.GetObstacleSurroundingPoints(obstacle_id,
                                                                                    -config_module.FLAGS_numerical_epsilon,
                                                                                    config_module.FLAGS_time_min_density)
        ...
        for path_time_point in follow_path_time_points:
            ...
            s_upper: float = path_time_point.s - config_module.FRONT_EDGE_TO_CENTER
            s_lower: float = s_upper - config_module.FLAGS_default_lon_buffer
            assert config_module.FLAGS_num_sample_follow_per_timestamp >= 2, "Number of sample follow per timestamp must be greater than 2"
            s_gap: float = config_module.FLAGS_default_lon_buffer / float(config_module.FLAGS_num_sample_follow_per_timestamp - 1)
            for i in range(config_module.FLAGS_num_sample_follow_per_timestamp):
                s: float = s_lower + s_gap * i
                sample_point: SamplePoint = SamplePoint(copy(path_time_point), v)
    ...
    def QueryOvertakePathTimePoints(self, obstacle_id: str, sample_points: List[SamplePoint]) -> None: 
        ...
        for path_time_point in over_take_path_time_points:
            v: float = self.prediction_querier.ProjectVelocityAlongReferenceLine(obstacle_id, path_time_point.s, path_time_point.t)
            sample_point: SamplePoint = SamplePoint(copy(path_time_point), v)
            sample_point.path_time_point.s = path_time_point.s + config_module.FLAGS_default_lon_buffer
            sample_points.append(sample_point)
